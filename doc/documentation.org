...
The resolution of =&= can be implemented like this.
#+begin_src clojure
(defn resolve-&
  [[_func & args]]
  (str/join ""
...
("|" "a" "b") => "(a|b)"
#+end_example
Grouping is applied as well. Since grouping is needed so often and the application is always the same, following helper function can be defined.
#+begin_src clojure
...
Following function places =|= between the arguments and groups it to the desired regular expression.
#+begin_src clojure
(defn resolve-|
  [[func arg1 arg2]]
  (string->regex-group
...
All of these operators can be resolved with this function.
#+begin_src clojure
(defn resolve-ops
  [[op arg]]
  (string->regex-group
